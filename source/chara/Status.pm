...
sub Init{
    ...
    $self->{CommonDatas}{CharacterJobName} = {}; # 戦型-習得可能技一覧で使用するため共用変数に記録
...
sub GetStatusData{
    ...
    foreach my $th_node (@$th_nodes){
        ...
        if($text eq "Lv"){
			...
			$th_node->right->as_text =~ m!(\d:)*(.+)/(\d:)*(.+)!;
            $job1 = $self->{CommonDatas}{ProperName}->GetOrAddId($2);
            $job2 = $self->{CommonDatas}{ProperName}->GetOrAddId($4);
    ...
    $self->{Datas}{Data}->AddData(join(ConstData::SPLIT, ($self->{ResultNo}, $self->{GenerateNo}, $self->{ENo}, $self->{SubNo}, $lv, $exp, $mexp, $job1, $job2, $hp, $mhp, $hp_rate, $mp, $mmp, $mp_rate, $sp, $str, $int, $tec, $agi, $def, $skill, $personality, $tribe, $money, $sundries) ));
