...
sub GetItemGetData{
    ...
    foreach my $item_get_div_node (@$item_get_div_nodes) {
        ...
        my $item_id = $self->{CommonDatas}{ProperName}->GetOrAddId($item_name);
        my $get_i_no  = 0;
        ...
        $self->{Datas}{Data}->AddData(join(ConstData::SPLIT, ($self->{ResultNo}, $self->{GenerateNo}, $self->{PartyNo}, $e_no, $sub_no, $enemy_id, $item_id, $get_i_no, $self->{IsPK}) ));
    }
