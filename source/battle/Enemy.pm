...
sub GetEnemyData{
    ...
    foreach my $bstat_table_node (@$bstat_table_nodes) {
        my $vspt_tr_nodes  = &GetNode::GetNode_Tag_Attr("tr", "class", "vspt", \$bstat_table_node);
        foreach my $vspt_tr_node (@$vspt_tr_nodes) {
            ...
            my $suffix_id = $self->{CommonDatas}{ProperName}->GetOrAddId($suffix);

            if (!exists($self->{Enemy}{$enemy_full_name})) { 
                $enemy_num += 1;

                $self->{Datas}{Enemy}->AddData(join(ConstData::SPLIT, ($self->{ResultNo}, $self->{GenerateNo}, $self->{PartyNo}, $enemy_id, $suffix_id) ));
            }
    ...
    $self->{Datas}{EnemyPartyInfo}->AddData(join(ConstData::SPLIT, ($self->{ResultNo}, $self->{GenerateNo}, $self->{PartyNo}, $enemy_num) ));
